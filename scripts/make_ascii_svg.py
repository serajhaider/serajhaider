...
def main():
    ...
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="760" height="{H}" viewBox="0 0 760 {H}"><rect width="100%" height="100%" rx="22" fill="#0d0d0d" stroke="#3b3320"/><circle cx="28" cy="24" r="6" fill="#ff5f57"/><circle cx="48" cy="24" r="6" fill="#febc2e"/><circle cx="68" cy="24" r="6" fill="#28c840"/><text x="100" y="29" fill="#aaa" font-family="monospace" font-size="14">The Cipher Stack / portrait</text><g fill="{GOLD}" font-family="monospace" font-size="11">{''.join(rows)}</g><style>@keyframes fin{{from{{opacity:0;transform:translateX(-8px)}}to{{opacity:1;transform:translateX(0)}}}}.ascii{{opacity:0;animation:fin .55s ease forwards}}@media(prefers-reduced-motion:reduce){{.ascii{{animation:none;opacity:1}}}}</style></svg>'''
