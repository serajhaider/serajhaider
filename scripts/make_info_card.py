...
def main():
    ...
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="900" height="420" viewBox="0 0 900 420"><rect width="100%" height="100%" rx="22" fill="#0d0d0d" stroke="#3b3320"/><circle cx="28" cy="24" r="6" fill="#ff5f57"/><circle cx="48" cy="24" r="6" fill="#febc2e"/><circle cx="68" cy="24" r="6" fill="#28c840"/><text x="100" y="29" fill="#aaa" font-family="monospace" font-size="14">The Cipher Stack</text><g font-family="monospace" font-size="18">{''.join(items)}</g><style>@keyframes fade{{from{{opacity:0;transform:translateY(5px)}}to{{opacity:1;transform:translateY(0)}}}}g g{{opacity:0;animation:fade .5s ease forwards}}</style></svg>'''
