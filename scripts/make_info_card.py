...
def main():
    ...
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="900" height="420" viewBox="0 0 900 420"><rect width="100%" height="100%" rx="22" fill="#0f172a" stroke="#2563eb"/><circle cx="28" cy="24" r="6" fill="#2563eb"/><circle cx="48" cy="24" r="6" fill="#3b82f6"/><circle cx="68" cy="24" r="6" fill="#7dd3fc"/><text x="100" y="29" fill="#bfdbfe" font-family="monospace" font-size="14">The Cipher Stack</text><g font-family="monospace" font-size="18"><style>.key{{fill:#7dd3fc}}.val{{fill:#dbeafe}}</style>{''.join(items)}</g><style>@keyframes fade{{from{{opacity:0;transform:translateY(5px)}}to{{opacity:1;transform:translateY(0)}}}}g g{{opacity:0;animation:fade .5s ease forwards}}</style></svg>'''
